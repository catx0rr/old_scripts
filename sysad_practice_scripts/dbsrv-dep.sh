...
host_name='webposdb'
pref_db='mariadb'
db_server_packages='mariadb-server'
ssh_port='5659'
admin_user='sysadmin'
...
nic_iface1='ens34'		# 1st NIC interface
nic_iface2='ens35'		# 2nd NIC interface
nic_iface3='ens36'		# 3rd NIC interface

# NIC Teaming Network Config
ipv4_address_wsubnet="192.168.248.100/24"
...
# Script Banner
banner1="${green}=============================>>${end} WELCOME to DATABASE SRV DEPLOYMENT ${green}<<=======================================${end}
					      SCRIPT!"
...
if [ $(id -u) != "0" ]; then
	...
	echo -e "${blue}>>${end} ${green}DONE${end}"
	echo -e "${blue}>>${end} ${yellow}Installing database server packages ..${end}"
	dnf install -y $pref_db $db_server_packages
	echo -e "${blue}>>${end} ${green}DONE${end}"
	echo -e "${blue}>>${end} ${yellow}Starting $pref_db service daemon ..${end}"
	
	# Services and Firewalls
	systemctl enable mariadb.service
	systemctl start mariadb.service
	echo -e "${blue}>>${end} ${green}DONE${end}"
	echo -e "${blue}>>${end} ${yellow}Configuring firewall for $pref_db service ..${end}"
	firewall-cmd --permanent --zone=public --add-service=mysql
	firewall-cmd --reload
	echo -e "${blue}>>${end} ${green}DONE${end}"
	echo -e "${blue}>>${end} ${yellow}Configuring SELinux for database server ..${end}"
...
finale(){
	...
	case $input in
   		[yY]|[yY][Ee][Ss]) 
				  ...
				  echo -en "${red}REBOOTING SYSTEM IN 3 SECONDS ..${end}"
				  sleep 3
				  systemctl reboot -i;;
	          [nN]|[n|N][O|o])
				  echo -e "${blue}>>${end} ${green}DONE${end}"
				  if [ "$pref_db" == "mariadb" ]; then
				  	dbsetup
				  else
				  	exit
				  fi;;
...
# Database setup
dbsetup(){
	echo -en "${blue}>>${end} ${yellow}Setting up password for $pref_db ..${end}"
	sleep 1
